...
public:
	...
	void setDataItem(MatrixXX&totError,std::unordered_map<IndexType,set<IndexType> >& edgePoints );
	void setSmoothItem();
...
public:
	...
	void bubleSort(vector<IndexType>& oriData,vector<IndexType>& labels,IndexType lSize);
	//void diff_using_bfs( std:: vector<IndexType>& labels,std::vector<IndexType>& centerVtxId,IndexType centerFrame );
